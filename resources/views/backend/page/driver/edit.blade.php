...
<div class="row owner-form">
	<div class="col-md-4 mt-5 ml-5">
		<form action="{{route('driver.update',$driver->id)}}" method="POST" class="needs-validation" novalidate>
         	@csrf
         	@method('PUT')
			...
			<div class="form-group">
				...
				<textarea name="address" id="" cols="30" rows="7" class="form-control {{ $errors->has('address') ? ' is-invalid' : '' }}"  id="address" value="" required>{{$driver->address}}</textarea>
